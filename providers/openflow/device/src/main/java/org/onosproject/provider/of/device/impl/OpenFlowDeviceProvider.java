...
public class OpenFlowDeviceProvider extends AbstractProvider implements DeviceProvider {
    ...
    // The Manager linking this provider to the core
    @Reference(cardinality = ReferenceCardinality.MANDATORY_UNARY)
    protected DeviceProviderRegistry providerRegistry;

    // For subscribing to OpenFlow switch-related events generated by the
    // controller. (Switch connected/disconnected, message arrived)
    @Reference(cardinality = ReferenceCardinality.MANDATORY_UNARY)
    ...
    // The interface defining how this provider interacts with the core
    ...
    private void connectInitialDevices() {
        // FIXME: does this clash with the wireless switch provider because it
        //        does not distinguish wired and wireless switches?
    ...
    public void changePortState(DeviceId deviceId, PortNumber portNumber,
        ...
        if (dev == null) {
            LOG.error("No device found with ID {}", deviceId);
            return;
        }
        OpenFlowSwitch sw = controller.getSwitch(dpid);
        ...
        pmb.setPortNo(port);
        if (!enable) {
            pmb.setConfig(ImmutableSet.of(OFPortConfig.PORT_DOWN));
        }
        pmb.setMask(ImmutableSet.of(OFPortConfig.PORT_DOWN));
    ...
    private class InternalDeviceProvider implements OpenFlowSwitchListener, OpenFlowEventListener {
        ...
        public void switchAdded(Dpid dpid) {
            ...
            // TODO: replace annotations with network config
...
//            }
        ...
        public void switchChanged(Dpid dpid) {
            ...
            if (!(sw instanceof OpenFlowWirelessSwitch)) {
//                ports = this.ethPortDescriptions((OpenFlowWirelessSwitch) sw);
                ports = Collections.emptyList();
            } else {
                ports = buildPortDescriptions(sw);
            }
        ...
        public void portChanged(Dpid dpid, OFPortStatus status) {
            LOG.debug("Port on switch {} changed: {}", dpid, status);
//            DeviceId devId = deviceId(uri(dpid));
//            OpenFlowSwitch sw = controller.getSwitch(dpid);
            PortDescription desc = null;
//
//            if (sw instanceof OpenFlowWirelessSwitch) {
////                desc = buildWirelessSwitchEthPortDescriptions(status);
//            } else {
//                desc = buildPortDescription(status);
//            }

            providerService.portStatusChanged(deviceId(uri(dpid)),
                                              buildPortDescription(status));
        }
        ...
        private PortDescription buildOduCltPortDescription(OFPortDesc port) {
            PortNumber portNo = portNumber(port.getPortNo().getPortNumber());
                ...
                switch (msg.getType()) {
                    case STATS_REPLY:
                        if (((OFStatsReply) msg).getStatsType() == OFStatsType.PORT) {
                            ...
                            List<PortDescription> portDescs = null;
                            if (sw instanceof OpenFlowOpticalSwitch) {
                                // Optical switch uses experimenter stats message to update power
                                portDescs = ((OpenFlowOpticalSwitch) sw).processExpPortStats(msg);
                            } else if (sw instanceof OpenFlowWirelessSwitch) {
                                // Wireless Switches use experimenter multipart messages to report Access Points
                                portDescs = ((OpenFlowWirelessSwitch) sw).handleExpMultipartReply(msg);
                            }

                            if (portDescs != null && !portDescs.isEmpty()) {
