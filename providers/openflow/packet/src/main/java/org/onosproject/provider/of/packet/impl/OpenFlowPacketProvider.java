...
import org.onosproject.net.packet.DefaultInboundWifiPacket;
...
import org.onosproject.openflow.controller.DefaultOpenFlowWifiPacketContext;
...
public class OpenFlowPacketProvider extends AbstractProvider implements PacketProvider {
    ...
    private class InternalPacketProvider implements PacketListener {
        ...
        public void handlePacket(OpenFlowPacketContext pktCtx) {
            ...
            InboundPacket inPkt;
            OutboundPacket outPkt = null;

            if (pktCtx instanceof DefaultOpenFlowWifiPacketContext) {

                DefaultOpenFlowWifiPacketContext ctx =
                        (DefaultOpenFlowWifiPacketContext) pktCtx;

                inPkt = new DefaultInboundWifiPacket(
                        new ConnectPoint(id, PortNumber.portNumber(pktCtx.inPort())),
                        ctx.parsed(), ByteBuffer.wrap(ctx.unparsed()), ctx.cookie());

                // TODO: outbound packet

            } else {
                inPkt = new DefaultInboundPacket(
                        new ConnectPoint(id, PortNumber.portNumber(pktCtx.inPort())),
                        pktCtx.parsed(), ByteBuffer.wrap(pktCtx.unparsed()), pktCtx.cookie());
            ...
            corePktCtx = new OpenFlowCorePacketContext(System.currentTimeMillis(),
