...
import org.projectfloodlight.openflow.protocol.OFExpExtAdIdPortOtn;
...
public class OplinkHandshakerUtil {
    ...
    // Total count of opspec in OFExpExtAdIdPortOtn
    ...
    private OplinkPortAdjacency getNeighbor(OFExpPortAdjacency ad) {
        ...
        for (OFExpPortAdjacencyId adid : ad.getProperties()) {
            ...
            if (otns != null && otns.size() > 0) {
                OFExpExtAdIdPortOtn otn = (OFExpExtAdIdPortOtn) otns.get(0);
                ...
                // |---------------------------------------------------------------------------|
                ByteBuf buffer = buffer(OPSPEC_BYTES);
